...
Question 3:
Component 1: Sovereign debt <br>
Component 2: Currencies<br>
Component 3: European indeces<br>
Component 4: North American indeces<br>
Component 5: European fx futures and securities <br>
Component 6: Oil <br>
Component 7: Industrial Metals <br>
Component 8: Soy, Corn and Wheat <br>
Component 9: Precious metals<br>
Component 10: Asian stock indeces<br>

<br> Question 4: 

4.a) the pre-crisis bull market years (e.g. only using the data between November 1, 2002, and October 1, 2007) 

Principal Components Analysis for Bull Years

```{r}
BullYears_data = futures_data[as.Date(rownames(futures_data)) > "2002-11-01" & as.Date(rownames(futures_data)) < "2007-10-01", ]
Variance_Explained_Table_results_BullYears<-PCA(bull_data, graph=FALSE)
Variance_Explained_Table_BullYears<-cbind(paste("component",1:ncol(bull_data),sep=" "),Variance_Explained_Table_results_BullYears$eig)
Variance_Explained_Table_BullYears<-as.data.frame(Variance_Explained_Table_BullYears)
colnames(Variance_Explained_Table_BullYears)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")


show_data_BullYears = data.frame(Variance_Explained_Table_BullYears)
m1_BullYears<-gvisTable(show_data_BullYears,options=list(width=1920, height=min(400,27*(nrow(show_data_BullYears)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))print(m1_BullYears,'chart')
```

Scree plot for Bull Years (2002-2007)
```{r}
eigenvalues_BullYears  <- Variance_Explained_Table_BullYears[,2]
df_BullYears           <- cbind(as.data.frame(eigenvalues_BullYears), c(1:length(eigenvalues_BullYears)), rep(1, length(eigenvalues_BullYears)))
colnames(df_BullYears) <- c("eigenvalues", "components", "abline")
Line_BullYears         <- gvisLineChart(as.data.frame(df_BullYears), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Screen plot (Bull Years)', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line_bull, 'chart')
```


Rotated principal components - Bull Years
```{r}
corused_BullYears = cor(BullYears_data[,apply(BullYears_data!=0,2,sum) > 10, drop=F])
Rotated_Results_BullYears<-principal(corused_BullYears, nfactors=10, rotate="varimax",score=TRUE)
Rotated_Factors_BullYears<-round(Rotated_Results_BullYears$loadings,2)
Rotated_Factors_BullYears<-as.data.frame(unclass(Rotated_Factors_BullYears))
colnames(Rotated_Factors_BullYears)<-paste("Component",1:ncol(Rotated_Factors_BullYears),sep=" ")

sorted_rows_BullYears <- sort(Rotated_Factors_BullYears[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors_BullYears <- Rotated_Factors_BullYears[sorted_rows,]
Rotated_Factors_BullYears[abs(Rotated_Factors_BullYears) < 0.3]<-NA
show_data_BullYears <- Rotated_Factors_BullYears
show_data_BullYears<-cbind(rownames(show_data_BullYears),show_data_BullYears)
colnames(show_data_BullYears)<-c("Variables",colnames(Rotated_Factors_BullYears))
m1_BullYears<-gvisTable(show_data_BullYears,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_BullYears)+1)),allowHTML=TRUE,page='disable'))
print(m1_BullYears,'chart')
```

b) the Crisis market years (e.g. only using the data between October 1, 2007 and March 1, 2009) 
Principal Components Analysis for Crisis Years
```{r}
CrisisYears_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]
Variance_Explained_Table_results_CrisisYears<-PCA(CrisisYears_data, graph=FALSE)
Variance_Explained_Table_CrisisYears<-cbind(paste("component",1:ncol(CrisisYears_data),sep=" "),Variance_Explained_Table_results_CrisisYears$eig)
Variance_Explained_Table_CrisisYears<-as.data.frame(Variance_Explained_Table_CrisisYears)
colnames(Variance_Explained_Table_CrisisYears)<-c("Component","Eigenvalue", "Percentage_of_explained_variance", "Cumulative_percentage_of_explained_variance")

show_data_CrisisYears = data.frame(Variance_Explained_Table_CrisisYears)
m1_CrisisYears<-gvisTable(show_data_CrisisYears,options=list(width=1920, height=min(400,27*(nrow(show_data_CrisisYears)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1_CrisisYears,'chart')
```
Screen plot for Crisis Years (2007-2009)
```{r}
eigenvalues_CrisisYears  <- Variance_Explained_Table_CrisisYears[,2]
df_CrisisYears           <- cbind(as.data.frame(eigenvalues_CrisisYears), c(1:length(eigenvalues_CrisisYears)), rep(1, length(eigenvalues_CrisisYears)))
colnames(df_CrisisYears) <- c("eigenvalues", "components", "abline")
Line_CrisisYears         <- gvisLineChart(as.data.frame(df_CrisisYears), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Screen plot (Crisis Years)', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line_CrisisYears, 'chart')
```
Rotated principal components - Crisis Years
```{r}
corused_CrisisYears = cor(CrisisYears_data[,apply(CrisisYears_data!=0,2,sum) > 10, drop=F])
Rotated_Results_CrisisYears<-principal(corused_CrisisYears, nfactors=10, rotate="varimax",score=TRUE)
Rotated_Factors_CrisisYears<-round(Rotated_Results_CrisisYears$loadings,2)
Rotated_Factors_CrisisYears<-as.data.frame(unclass(Rotated_Factors_CrisisYears))
colnames(Rotated_Factors_CrisisYears)<-paste("Component",1:ncol(Rotated_Factors_CrisisYears),sep=" ")

sorted_rows_CrisisYears <- sort(Rotated_Factors_CrisisYears[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors_CrisisYears <- Rotated_Factors_CrisisYears[sorted_rows,]
Rotated_Factors_CrisisYears[abs(Rotated_Factors_CrisisYears) < 0.3]<-NA
show_data_CrisisYears <- Rotated_Factors_CrisisYears 
show_data_CrisisYears<-cbind(rownames(show_data_CrisisYears),show_data_CrisisYears)
colnames(show_data_CrisisYears)<-c("Variables",colnames(Rotated_Factors_CrisisYears))
m1_CrisisYears<-gvisTable(show_data_CrisisYears,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_crisis)+1)),allowHTML=TRUE,page='disable'))
print(m1_CrisisYears,'chart')
```
<br>
Question 5: 

First of all we can see that the number of components required to explain >50% of the variance for each dataset is different. In the crisis years we observe that fewer components explain a higher percentage of variance compared to the bull market years, implying that assets become more highly correlated to each other. A well diversified portfolio is therefore more important in a bear market.
...
**Your Answers here:**
<br> Question 1: I found the description in the comments above sufficiently detailed. 

Question 2: 
When I use different moving average technical indicators in the code above I get different returns. More specifically increasing the MAfreq sequence increment increases the returns but gives me higher volatility too.  
