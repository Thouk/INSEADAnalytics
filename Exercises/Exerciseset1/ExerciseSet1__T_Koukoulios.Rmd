...
**Your Answers here:**
<br> Question 1: In the dataSet1.R file the tickers are set to ("SPY", "AAPL", "YHOO") on ine 8 of the code. The tickers are then used in the loop starting on line 20 which loops through the tickers on line 8 and downlowades the corresponding data.

<br>
Question 2: 
Apple:

- Apple cumulative returs: `r round(100*sum(StockReturns[,2]),1)`%. <br>
- Apple average daily returns of the `r round(100*mean(StockReturns[,2]),3)`%;<br>
- Standard deviation of Apple daily returns: `r round(100*sd(StockReturns[,2]),3))`%;<br>
<br>
<br>
Yahoo: <br>
- Yahoo cumulative returs: `r round(100*sum(StockReturns[,3]),1)`%. <br>
- Yahoo average daily returns of the `r round(100*mean(StockReturns[,3]),3)`%;<br>
- Standard deviation of Yahoo daily returns: `r round(100*sd(StockReturns[,3]),3))`%;<br>
<br>
...
**Your Answers here:**
<br> Question1: `nrow(transposedData)` and `ncol(transposedData)`
<br> Question 2:
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
transposedData = t(StockReturns[,1:2])
...
**Your Answers here:**
<br> Question1: We would change the startDate parameter in line 9 of the code in the dataSet1.R file. (it is already set to 2001-01-01) 
<br> 
<br>
<br>
...
```
<br>
The command formats the data when saving and retrieving from .csv file. The data is table in a table format with 1 column and 20 lignes.
